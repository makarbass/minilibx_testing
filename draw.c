...
void bresenham(float x, float y, float x1, float y1, fdf *data) // рисование линии ( передаём координаты начала и конца, и структуру)
{
	...
	{
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, x, y, 0x00FF0000); // рисуем пиксель
		x += x_step; // смещаемся по Х
		y += y_step; // смещаемся по Y
	}
}

void square(fdf *data) // для рисования квадрата передаем структуру
{
	int x;
	int y;
	...
	while (x < data->zoom) // пока Х меньше коэфицента зума
	{
		...
		{
			mlx_pixel_put(data->mlx_ptr, data->win_ptr, x + data->x, y + data->y, 0x00FF0000); // рисуем точку
